...
Stock `kanagawa-lotus` fails WCAG AA contrast on most of its syntax
colors against its own background. This variant keeps the same soft,
...
hue so they don't read as washed-out. See [PALETTE.md](PALETTE.md) for
the full color table, the reasoning behind each change, and an honest
accounting of which colors are genuinely sampled from the source painting
versus invented for legibility.
